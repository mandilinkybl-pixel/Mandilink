...
class MandiController {
 ...
 async getMandis(req, res) {
  try {
    ...
    // Fetch all mandis with populated state and user details
    const mandis = await Mandi.find(filter)
      .populate("state", "name") // populate state name
      .populate("user_id", "name email contactNumber role") // populate user details from SecureEmployee
      .lean();
    ...
    res.status(200).json({
      ...
      mandis: mandis.map(m => ({
        ...
        district: m.district,
        state: m.state?.name || "",
        user: m.user_id || {}, // full SecureEmployee details
