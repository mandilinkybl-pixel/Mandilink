...
const Company = require("../../models/companylisting"); // Company model

const JWT_SECRET = process.env.JWT_SECRET || "mysecretkey";

class AuthController {

  // ✅ Signup (all fields in one request)
  ...
  async login(req, res) {
    try {
      ...
      let account = await User.findOne({ $or: [{ email: identifier }, { contactNumber: identifier }] }) ||
      ...
      const token = jwt.sign({ id: account._id, userType }, JWT_SECRET, { expiresIn: "7d" });
  ...
  async forgotPassword(req, res) {
    try {
      ...
      const otp = "123456"; // Replace with real OTP
